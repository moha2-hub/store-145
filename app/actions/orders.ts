...
function getOrderSelectQuery(role: string) {
  const baseFields = `
    ...
    json_build_object('name', c.name${role !== "customer" ? ", 'login_credentials', c.login_credentials" : ""}) as castle
  `
...
export async function createOrder(formData: FormData) {
  ...
  try {
    ...
    const insertRes = await query(
      `INSERT INTO orders (customer_id, product_id, castle_id, status, amount, quantity, created_at, updated_at)
       VALUES ($1, $2, $3, 'pending', $4, $5, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) RETURNING id`,
      [user.id, productId, castleId, totalPrice, quantity]
    )
