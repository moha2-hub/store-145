...
export async function createCastle(formData: FormData) {
  ...
  const iggId = formData.get("iggId") as string
  const castleId = formData.get("castleId") as string
  const loginCredentials = formData.get("loginCredentials") as string

  try {
    const result = await query(
      `INSERT INTO castles (user_id, name, igg_id, castle_id, login_credentials) 
       VALUES ($1, $2, $3, $4, $5) RETURNING id`,
      [user.id, name, iggId, castleId, loginCredentials],
    )
